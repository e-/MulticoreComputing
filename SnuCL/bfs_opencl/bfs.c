...
cl_mem	xadj_buffer[MAX_DEV], 
		xoff_buffer[MAX_DEV],
		global_state_buffer, 
		global_bfs_tree_buffer,
		finished_buffer[MAX_DEV];
char *state[MAX_DEV];
char *global_state, *finished;
int *global_bfs_tree;
...
"__kernel void step1(__global const int *xadj,__global const int *xoff, __global char *state, __global long *bfs_tree, const int maxvtx, const int offset, const int xadj_offset){" 
...
//	USE CPU
//	err = clGetDeviceIDs(platform, CL_DEVICE_TYPE_CPU, 0, NULL, (unsigned int*)&ndev); 
//	err = clGetDeviceIDs(platform, CL_DEVICE_TYPE_CPU, ndev, device, NULL);
...
//	USE GPU + CPU
	...
	global_state_buffer = clCreateBuffer(context, CL_MEM_READ_WRITE | CL_MEM_USE_HOST_PTR, global_state_buffer_size, global_state, &err);
	       ...
	       int64_t srcvtx)
{
	...
	*max_vtx_out = maxvtx;
	...
	global_bfs_tree_buffer = clCreateBuffer(context, CL_MEM_WRITE_ONLY | CL_MEM_USE_HOST_PTR, global_bfs_tree_buffer_size, bfs_tree, &err);
	bfs_tree[srcvtx] = srcvtx;
	...
	while(1){
		for(i=0;i<ndev;++i) {
			...
			err = clSetKernelArg(kernel1[i], 2, sizeof(cl_mem), (void *)&global_state_buffer);
			err = clSetKernelArg(kernel1[i], 3, sizeof(cl_mem), (void *)&global_bfs_tree_buffer);
		...
		for(i=0;i<ndev;++i){
			err = clSetKernelArg(kernel2[i], 0, sizeof(cl_mem), (void *)&global_state_buffer);
			err = clSetKernelArg(kernel2[i], 1, sizeof(cl_mem), (void *)&finished_buffer[i]);
			err = clSetKernelArg(kernel2[i], 2, sizeof(maxvtx), (void *)&maxvtx);
			err = clSetKernelArg(kernel2[i], 3, sizeof(start[i]), (void *)&start[i]);
			err = clEnqueueNDRangeKernel(command_queue[i], kernel2[i], 1, 0, global[i], local[i], 0, 0, NULL);
			clEnqueueReadBuffer(command_queue[i], finished_buffer[i], CL_FALSE, 0, finished_buffer_size, &finished[i], 0, 0, 0);
		}
	...
	clReleaseMemObject(global_bfs_tree_buffer);
	return err;
}
